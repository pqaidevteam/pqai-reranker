"""Models for reranking, that take as input a set 
of documents and ranks them on the basis of their extent of similarity to a query.
"""
...
class ConceptMatchRanker(Ranker):
    ...
    def _get_similarity(self, qry, doc):
        qry_boe = encode(qry, "boe")
        doc_boe = encode(doc, "boe")
        qry_rep = encode(qry_boe, "emb")
        doc_rep = encode(doc_boe, "emb")
        return self._wmd(qry_rep, doc_rep)

    def _wmd(self, bov1, bov2, dist_fn=distance.cosine):
        n1 = len(bov1)
        n2 = len(bov2)
        if n1 == 0 or n2 == 0:
            return math.inf
